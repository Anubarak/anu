...
use anu\db\ActiveRecordInterface;
use anu\helper\Inflector;
...
class UniqueValidator extends Validator
{
    ...
    public $targetClass;
    /**
     * @var string|array the name of the [[\anu\db\ActiveRecord|ActiveRecord]] attribute that should be used to
     ...
     * This can be a string or an array representing the additional query condition (refer to [[\anu\db\Query::where()]]
     * on the format of query condition), or an anonymous function with the signature `function ($query)`, where `$query`
     * is the [[\anu\db\Query|Query]] object that you can modify in the function.
     */
    public $filter;
    /**
    ...
    public function init()
    {
        ...
        if (is_array($this->targetAttribute) && count($this->targetAttribute) > 1) {
            ...
            if ($this->comboNotUnique === null) {
                $this->message = Anu::t('anu', 'The combination {values} of {attributes} has already been taken.');
            } else {
                $this->message = $this->comboNotUnique;
            }
        } else {
            $this->message = Anu::t('anu', '{attribute} "{value}" has already been taken.');
        }
    }
    ...
    public function validateAttribute($model, $attribute)
    {
        ...
        foreach ($rawConditions as $key => $value) {
            if (is_array($value)) {
                $this->addError($model, $attribute, Anu::t('anu', '{attribute} is invalid.'));
                return;
            }
     ...
     * @param array $conditions conditions, compatible with [[\anu\db\Query::where()|Query::where()]] key-value format.
     * @param SavableComponent $model the data model to be validated
     *
    ...
    private function modelExists($targetClass, $conditions, $model)
    {
        ...
        if ($query instanceof \anu\db\ActiveQuery) {
     ...
     * [[\anu\db\Query::where()|Query::where()]] key-value format.
     *
     * @param string|array $targetAttribute the name of the [[\anu\db\ActiveRecord|ActiveRecord]] attribute that
     ...
     *
     * @return array conditions, compatible with [[\anu\db\Query::where()|Query::where()]] key-value format.
     */
    private function prepareConditions($targetAttribute, $model, $attribute)
    {
        ...
        if (!is_subclass_of($targetModelClass, 'anu\db\ActiveRecord')) {
            return $conditions;
        }
     ...
     * @param \anu\base\Model $model the data model.
