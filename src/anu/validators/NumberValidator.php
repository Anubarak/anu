...
class NumberValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if ($this->message === null) {
            $this->message = $this->integerOnly ? Anu::t('anu', '{attribute} must be an integer.')
                : Anu::t('nu', '{attribute} must be a number.');
        }
        if ($this->min !== null && $this->tooSmall === null) {
            $this->tooSmall = Anu::t('anu', '{attribute} must be no less than {min}.');
        }
        if ($this->max !== null && $this->tooBig === null) {
            $this->tooBig = Anu::t('anu', '{attribute} must be no greater than {max}.');
        }
    }
    ...
    protected function validateValue($value)
    {
        if (is_array($value) || is_object($value)) {
            return [Anu::t('anu', '{attribute} is invalid.'), []];
        }
        $pattern = $this->integerOnly ? $this->integerPattern : $this->numberPattern;
        if (!preg_match($pattern, StringHelper::normalizeNumber($value))) {
            return [$this->message, []];
    ...
    public function clientValidateAttribute($model, $attribute, $view)
    {
        ...
        return 'anu.validation.number(value, messages, ' . json_encode($options) . ');';
    }
