...
PATTERN;
     ...
     * $query = (new \anu\db\Query)->from(['u' => 'user', 'profile']);
     *
     * // SELECT * FROM (SELECT * FROM `user` WHERE `active` = 1) `activeusers`;
     * $subquery = (new \anu\db\Query)->from('user')->where(['active' => true])
     * $query = (new \anu\db\Query)->from(['activeusers' => $subquery]);
     *
